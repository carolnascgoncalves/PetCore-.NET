...
public class EnderecoController(IEnderecoService enderecoService) : ControllerBase
{
    ...
    {
        return Ok(enderecoService.FetchAll());
    }
    ...
    {
        var item = enderecoService.FetchById(id);
    ...
    [ProducesResponseType(StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    public IActionResult Patch(Guid id, [FromBody] EnderecoRequest request)
    {
        var updated = enderecoService.Patch(id, request);
    ...
    {
        var deleted = enderecoService.Delete(id);
