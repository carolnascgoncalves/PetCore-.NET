...
public class ClinicaController(IClinicaService clinicaService) : ControllerBase
{
    ...
    {
        return Ok(clinicaService.FetchAll());
    }
    ...
    {
        var item = clinicaService.FetchById(id);
    ...
    [ProducesResponseType(StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    public IActionResult Patch(Guid id, [FromBody] ClinicaDadosRequest request)
    {
        var updated = clinicaService.Patch(id, request);
    ...
    {
        var deleted = clinicaService.Delete(id);
